...
# basic report: there are a few loops and spagheti in the code, either re-write it, they are probably alright
# secondary roport, change the coreX or coreY and imageX and imageY system, it does nothing 
# TODO: proposal on chenging the system will be universal rect x and y cords but other variables are in place to calculate how far of it is from the rect.
...
class Label(Area):
    ...
    def set_Label(self, text, tsize=12, tcolor=BLACK, tbold=False):
        ...
        self.tbold = tbold
        self.tcolor = tcolor
        self.font = pygame.font.SysFont('verdana', tsize, tbold).render(('score:' + str(self.score)), True, tcolor)
    ...
    def draw_Label(self, shift_x=0, shift_y=0,fillbfr=False):
        ...
        if fillbfr == True:
            self.fill()
        self.font = pygame.font.SysFont('verdana', self.tsize, self.tbold).render(('score:' + str(self.score)), True, self.tcolor)
...
class Entity(Area):
    ...
    def display_current_anim(self, target_fps : float, fps : float, frames : int=12 ):
        ...
        if self.currentAnim != '': 
            ...
            if fps > 0:
                ...
                self.AnimData += frames/target_fps
    ...
    def debug(self, window:pygame.Surface, info): # TODO Make it into a Class
        self.debug_text = []
...
# DEBUG _____________________________________________# TODO
